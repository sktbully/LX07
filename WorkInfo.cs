...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过' and recommend='是' order by recommedTime desc limit 6";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public int GetCount(int[] activityIds)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                return cn.ExecuteScalar<int>(sql, new { actvityIds = activityIds});
            }
        }
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                if (workinfo.workPicture == null)
                    sql = "update workinfo set workIntroduction=@workIntroduction whre workId=@workId";
                return cn.Execute(sql, workinfo);
            }
        }
